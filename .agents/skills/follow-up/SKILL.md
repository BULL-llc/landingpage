...
- 永続バックログ: `.Codex/state/follow-ups.jsonl`
...
- 初回利用時は `.Codex/state/` ディレクトリと空のファイルを作る (`mkdir -p .Codex/state && touch .Codex/state/follow-ups.jsonl`)
...
```bash
# 実装イメージ
[ -f .Codex/state/follow-ups.jsonl ] || { echo "（フォローアップなし）"; exit 0; }
jq -c 'select(.status == "open")' .Codex/state/follow-ups.jsonl | \
...
jq -c --arg id "$ID" --arg pr "$PR_URL" --arg now "$(date -u +"%Y-%m-%dT%H:%M:%SZ")" \
  ...
  .Codex/state/follow-ups.jsonl > "$TMP"
mv "$TMP" .Codex/state/follow-ups.jsonl
```
...
echo ""
jq -c --arg b "$BRANCH" 'select(.status == "open" and .branch == $b)' .Codex/state/follow-ups.jsonl 2>/dev/null | \
...
`.Codex/state/follow-ups.jsonl` から `status: resolved` を抜いて `.Codex/state/follow-ups.archive.jsonl` に移す。バックログが肥大化したときの掃除用。
...
Codex: ID F-A1B2C3 で記録した。PR 本文に貼る markdown:
        ## Known follow-ups
...
Codex: F-A1B2C3 を resolved に更新した。
```
